...
В случаях, когда $k=2$ и $k=3$, мы называем $K$ \emph{фрактальным квадратом} и \emph{фрактальным кубом} соответственно.
...
%Для этого требуется удалить из графа вершины, которым соответствуют пустые $F_\bma$, а также удалить те рёбра $(F_\bma,F_\bmb)$ при $\bmb\sqsupseteq\bma$, для которых $G_{\bma\bmb}=\0$ или $F_\bmb=\0$.
...
\begin{definition}\label{dfn:SGpath}
...
%Исходя из теоремы \ref{IFC}, граф $\Ga_\bma$ задаёт множество всех уравнений, которые полностью определяют каждое из множеств $F_\bmb$, для которых $\bmb\succcurlyeq\bma$.
...
%Этой цепочке соответствует композиция $T_{\bma_1\bma_2} \circ \ldots \circ  T_{\bma_{p-1}\bma_p}$.
...
Если $\#B=1$, то $B=\dfrac{F_\bma+G_{0\bma}}{n}$ для некоторого $\bma\succ0$, где $\#F_\bma=1$ и $G_{0\bma}=\{d_{0\bma}\}$.
$$\dfrac{F_\bma+d_{0\bma}}{n}=\dfrac{d_0}{n-1}
\quad\Rightarrow\quad
F_\bma=\dfrac{nd_0-(n-1)d_{0\bma}}{n-1}.$$
При $(d_0)_i\neq(d_{0\bma})_i$ получаем $(F_\bma)_i\notin[0,1]$, поэтому $d_0=d_{0\bma}$ и $F_\bma=\dfrac{d_{\bma0}}{n-1}$.
Из последнего равенства следует, что $G_0=G_{0\bma}=G_\bma=\{d_0\}$.
Следовательно, $\{d_0,\ d_0-(n-1)\bma,\ d_0+\bma\}\IN D^2$, что возможно только при $\bma=0$.
...
\begin{example}[Пересечение двух фрактальных квадратов, состоящее из $24$ точек, и его структурный граф $\Ga_\Sa$] \label{ex:3_1}

Рассмотрим пересечение фрактальных квадратов $K_1$ и $K_2$ порядка $6$ с множествами единиц $D_1$ и $D_2$.
На рисунке \ref{fig:FSI_6x6_DSK} слева показаны $D_1$ и $D_2$ множеством красных квадратов $T_1(P)=\dfrac{D_1+P}{6}$ и синих квадратов $T_2(P)=\dfrac{D_2+P}{6}$.
На этом же рисунке справа показаны фрактальные квадраты $K_1$ и $K_2$. \\

Большинство из множеств $G_\bma$, а именно, 
$G_0$, $G_{(1,0)}$, $G_{(-1,0)}$, $G_{(0,1)}$, $G_{(0,-1)}$, $G_{(1,1)}$, $G_{(1,-1)}$ и $G_{(-1,1)}$ являются пустыми, и только. 
Следовательно, множества $F_{(1,1)}$, $F_{(1,-1)}$, $F_{(-1,1)}$, $F_{(1,0)}$, $F_{(0,1)}$ --- пусты.
...
В структурном графе $\Ga_\Sa$ (см. рисунок \ref{fig:FSI_6x6_SG}) содержатся только вершины $0$, ${(-1,0)}$, ${(0,-1)}$, ${(-1,-1)}$.
Из $G_{(-1,-1)}=\{(0,0)\}$ получаем одноточечное $F_{(-1,-1)}=\{(0,0)\}$.

Легко вычислить, что $\#G_{(-1,0)(-1,-1)}=\#G_{(0,-1)(-1,-1)}=2$,\ $\#G_{0(0,-1)}=\#G_{0(-1,0)}=6$ и $G_{0(-1,-1)}=\0$.
Поэтому, согласно теореме \ref{fin_int},
$\#F_{(-1,0)}=\#F_{(0,-1)}=2$ и 
$$\#F_0=\#F_{(-1,0)}\cdot\#G_{0(-1,0)}+\#F_{(0,-1)}\cdot\#G_{0(0,-1)}=24.$$

\begin{figure}[H]
    \centering
    \includegraphics[width=.45\textwidth]{FSI_6x6_SG_new.pdf}
    \caption{Структурный граф $\Ga_\Sa=\Ga_0$ и его подграфы $\Ga_{(0,-1)}$ и $\Ga_{(-1,0)}$}
    \label{fig:FSI_6x6_SG}
\end{figure} 
\end{example} 
...
\section{Фрактальные кубы с одноточечным пересечением, являющиеся дендритами}
...
Поэтому критическое множество фрактального $k$-куба задаётся равенством 
...
Тогда для каждого $\bma\in A_D$ каждая пара белых вершин $d,\ d+\bma\in D$ графе $\hat\Gamma$ соединена рёбрами с общей чёрной вершиной, которой которой соответствует множество $K_{(d)}\cap K_{(d+\bma)}$.
...
%На рисунке \ref{fig:triple} выделены две тройки копий во фрактальном квадрате с одноточечным пересечением.
...
Теорема \ref{thm:triple} естественным образом обобщается для случая, в котором $m$ копий имеют общую точку.

\begin{corollary}\label{mpoint}
Пусть $ B=\{d_1,...,d_m\}$ --- такое подмножество в $D$, что для любых $d_i,d_j,d_k\in B$ справедливо 
...
\item Согласно теореме \ref{IFC}, найдём все множества $G_\bma, G_{\bma\bmb}$ и выпишем систему уравнений $\Sigma$.
...
%Для этого векторам из множества единиц $D$ ставим в соответствие белым вершинам графа $\hat\Gamma$.
%Для каждого $d\in G_{0\bma}$ пару белых вершин, соответствующих векторам $d$ $d+\bma$, соединим рёбрами с чёрной вершиной, соответствующей точке $\{x\}=\dfrac{F_\bma+d}{n}$. 
