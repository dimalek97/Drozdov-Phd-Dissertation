...
Между множеством $A$ и множеством граней единичного $k$-куба $P=[0,1]^k$ существует следующее взаимно-однозначное соответствие:
...
Символом $F_\bma$ обозначим пересечение $K_\bma\cap(K_{-\bma}+\bma)$ пары противоположных граней фрактального $k$-куба.
\end{definition}
...
Так как $d_\bj-d_\bi\in\zz^2$, %по Определению \ref{refin}
последнее пересечение может быть непусто только в том случае, если $d_\bj-d_\bi$ равно некоторому $\bma\in A$. 
...
Любое из этих пересечений является образом множества $F_{(1,0)}, F_{(0,1)}, F_{(1,1)}$ или $ F_{(-1,1)}$ относительно некоторого отображения $S_\bi$. 
Множества $F_{(1,1)}$ и $ F_{(-1,1)}$ соответствуют пересечениям противоположных углов, а потому являются одноточечными или пустыми. 

Для любых $i,j\in\{-1,1\}$ множество $F_{(i,j)}=\left(\frac{i+1}{2},\frac{j+1}{2}\right)$, если $(n-1)\cdot\left(\frac{i+1}{2},\frac{j+1}{2}\right)\in D_1$ и $(n-1)\cdot\left(\frac{-i+1}{2},\frac{-j+1}{2}\right)\in D_2$, в противном случае $F_{(i,j)}=\0$. 

Множества $F_{(1,0)}, F_{(0,1)}$ соответствуют вертикальному и горизонтальному соседству копий. 
Каждое из этих множеств является пересечением противоположных сторон фрактального квадрата $K$.
...
\begin{proof}
По Предложению \ref{fbma}(1),
\begin{multline}\label{line}
nF_\bma=nK_\bma\cap (nK_{-\bma}+n\bma)=nK\cap(nK+n\bma)=\\
=(K+D)\cap(K+D+n\bma)=
...
$$(K+d_1)\cap (K+d_2+n\bma)= (K\cap(K+\bmb))+d_1= F_\bmb+d_1,$$ 
