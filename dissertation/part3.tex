...
\begin{definition}
{\em Критическое множество} аттрактора $K$ системы $\mathcal{S}$ --- это множество $C:=\{x:\; x\in S_i(K)\cap S_j(K),\; S_i, S_j\in\mathcal{S}\}$ точек попарных пересечений копий $K$. 
\end{definition}


\begin{definition}
Множесво $\dd K$ всех $x\in K$ таких, что для некоторого $\bj\in I^*$ верно $S_\bj(x)\in C$, называется {\em самоподобной границей} множества $K$ или его .
\end{definition}
...
\begin{figure}[H]
 ...
 \caption{Фрактальный квадрат (справа) множество $D+P$ (слева). Элементы из $D$ отмечены чёрными точками.}
 \label{fig:fr_sq}
\end{figure}

Уравнение \eqref{fsqeq} определяет $\eS$ гомотетий $S_i(x)=\dfrac{x+d_i}{n}$, где $i\in I=\{1, \ldots, m\}$, а оператор Хатчинсона $T_\eS$ системы $\eS$ определяется уравнением $T_\eS(A)=\dfrac{D+A}{n}$.

\begin{definition}\label{refin}
 Для любого $k\in \nn$, {\em $k$-е измельчение} системы $\eS$ --- это система $\eS^k=\{S_\bi, \bi\in I^k\}$, где $S_\bi(x)=\dfrac{x+d_\bi}{n^k}$ и $d_\bi=n^{k-1}d_{i_1}+n^{k-2}d_{i_2}+\ldots+d_{i_k}$. 
\end{definition}

Сисиема $\eS^k$ представляет $K$ как фрактальный куб порядка $n^k$ с множеством единиц $D^k=n^{k-1}D+n^{k-2}D+\ldots+D$ и копиями $\dfrac{K+d_\bi}{n^k}$.
Оператор Хатчинсона $T_\eS^k$ этой системы $\eS^k$ определяется уравнением $T_\eS^k(A)=\dfrac{D^k+A}{n^k}$.

Каждой бесконечной строке $\bi=i_1i_2\ldots\in I^\8$ соответствует единственная точка $x=\pi(\bi)$ по формуле $\pi(\bi)=\sum\limits_{k=1}^n \dfrac{d_{i_k}}{n^k}$.
...
% Аналогичным образом {\em <<фрактальные квадраты $K$ порядка $n$ множеством единиц $D$, являющиеся дендритами>>} мы будем называть как <<фрактальные квадраты дендриты>> или использовать аббревиатуру <<FSD>>. 
\end{remark}


\section{Пересечения копий фрактального квадрата}

\red{In {\bf 3.1} we consider the face intersection sets $F_\bma$ (Definition \ref{def-falpha}) and show that the pieces of a fractal square intersect by homothetic images of these sets $F_\bma$ (Proposition \ref{fbma}) and find the equations for these sets (Theorem \ref{thm:falpha}).
In {\bf 3.2} we obtain the conditions under which the side intersection sets $F_\bma$ are singletons
(Corollary \ref{onepoint}).
}
...
\begin{definition}
Каждый $\bma\in A=\{-1,0,1\}^2$определяет единственную грань $P_\bma$ единичного квадрата $P$ согласно равенству $P_\bma=P\cap(P+\bma)$.
\end{definition}
...
\red{
There is an order relation $\sqsubseteq$ on the set $A$ which is order isomorphic to the inclusion ordering $\supseteq$ of the faces of $P$.}
...
\begin{proof}По Определению \ref{def-falpha},
...
Беря $i$-ю для всех записей последнего равенства, мы получаем, что
...
\subsection{Мощность пересечений фрактальных отрезков}
...
тип {\bf C} если $\bma=(1,1) \mbox{ or } (1,-1),\bmb=(1,0) \mbox{ or }(0,1)$.\\
...
Для являющегося дендритом фрактального квадрата каждого типа в множестве единиц существуют комбинации единиц, которые не могут встречаться в $D^k$ для любого $k$. 
...
\begin{figure}[H]
    ...
    \caption{A corner point of order 2 for the type {\bf C} and two main arcs containing it.} 
...
\begin{figure}[H]
    \centering \includegraphics[width=0.8\textwidth]{d6ord3_12.pdf}
    % \caption{Caption}
    % \label{fig:d6ord3_12}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Classes 1 and 2. The tree types are \textbf{1-A} and \textbf{1-C}. }
    \label{fig:tree1}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Classes 3 ({\bf 2-A}), 4 ({\bf 2-A}), 5 ({\bf 2-D6}) and 6 ({\bf 2-D3})}
    \label{fig:tree2}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Classes 7({\bf 3-A}), 8 ({\bf 3-B}) и 9({\bf 3-C})}
    \label{fig:tree3}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Classes 10 ({\bf 4-A}), 11 ({\bf 4-B}), 12 ({\bf 4-C}) and 13 ({\bf 4-D6})}
    \label{fig:tree4}
\end{figure}
