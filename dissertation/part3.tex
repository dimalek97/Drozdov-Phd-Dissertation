...
\item[(1)] Если существует $\bmb\succcurlyeq\bma$ такое, что $\#G_\bmb>1$, то множество $F_\bma$ несчетно.

\item[(2)] Если для любого $\bmb\succcurlyeq\bma$ верно неравенство $\#G_\bmb\leq1$, то множество $F_\bma$ не более чем счетно.

\item[(3)] Если для всех максимальных вершин $\bmb$ в $\Gamma_\bma$ верно $\#G_\bmb=1$, а для всех остальных вершин $\bma_i$ в $\Gamma_\bma$ верно $G_{\bma_i}=\0$, то множество $F_\bma$ конечно.
В этом случае $\#F_\bma$ не превосходит сумму всех композиций $\prod \limits_{j=1}^{p-1}\# G_{\bma_j\bma_{j+1}}$, взятых по всем цепочкам $\bma=\bma_1\prec\ldots\prec\bma_p=\bmb$, где $\bmb$ является максимальным в $\Gamma_\bma$.

\item[(4)] Если все $\bma_i\succcurlyeq\bma$ образуют единственную цепочку $\bma=\bma_1\prec\ldots\prec\bma_p$, в которой для всех $j\le p-1$ верны равенства $\# G_{\bma_j\bma_{j+1}}=1$, $G_{\bma_j}=\0$ и $\#G_{\bma_p}=1$, то множество $F_\bma$ является одноточечным.
\end{enumerate}
\end{theorem}
...
(2)
Обозначим $B:=\bigcup_{\bma\sqsupset0}\dfrac{G_{0\bma}+F_\bma}{n}$.
Для $F_0$ справедливо выражение $F_0 = \dfrac{G_0+F_0}{n}\cup B$.
Пусть $B$ не пусто и более чем счётно.
...
В этом случае $F_0$ счётно, только если $B\neq\dfrac{d_0}{n-1}.$
