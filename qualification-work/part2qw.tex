...
\subsection{Главное дерево}

\begin{figure}[H]
\center{
\includegraphics[width=0.5\textwidth]{penta4plus3.png}}
\caption{Полигональный дендрит и его главное дерево}
\label{fig:penta4plus3}
\end{figure}
Вспомним определения критического множества и самоподобной границы из Главы 1. 
...
\begin{definition}
Множество $\dd K$ всех $x\in K$ таких, что для некоторого $\bj\in I^*$ верно $S_\bj(x)\in C$, называется {\em самоподобной границей} множества $K$ или его .
\end{definition}
...
Уравнение \eqref{fsqeq} определяет систему $\eS$ гомотетий $S_i(x)=\dfrac{x+d_i}{n}$, где $i\in I=\{1, \ldots, m\}$, а оператор Хатчинсона $T_\eS$ системы $\eS$ определяется уравнением $T_\eS(A)=\dfrac{D+A}{n}$.
